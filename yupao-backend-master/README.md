...
### 环境信息
- **环境 ID**: `cloud-base-jh-7gdrxr7a9b8551a5`
- **环境别名**: cloud-base-jh
- **区域**: ap-shanghai

### 服务地址
- **后端 API**: https://yupao-backend-244824-5-1320287214.sh.run.tcloudbase.com
- **前端页面**: https://cloud-base-jh-7gdrxr7a9b8551a5-1320287214.tcloudbaseapp.com/

### 使用的 CloudBase 资源
| 资源 | 类型 | 说明 |
|------|------|------|
| 云托管 | 容器型 (Java 8) | 后端 Spring Boot 服务，0.5核1G，最小1实例 |
| 静态托管 | — | 前端 Vue 3 构建产物 |
| MySQL | 需在控制台启用 | 数据库服务（需手动配置连接） |
| Redis | 需自行部署或使用云 Redis | Session 和缓存（需手动配置连接） |

### 控制台入口
- 概览: https://tcb.cloud.tencent.com/dev?envId=cloud-base-jh-7gdrxr7a9b8551a5#/overview
- 云托管: https://tcb.cloud.tencent.com/dev?envId=cloud-base-jh-7gdrxr7a9b8551a5#/platform-run
- 静态托管: https://tcb.cloud.tencent.com/dev?envId=cloud-base-jh-7gdrxr7a9b8551a5#/static-hosting
- MySQL: https://tcb.cloud.tencent.com/dev?envId=cloud-base-jh-7gdrxr7a9b8551a5#/db/mysql

### 注意事项
1. 后端环境变量中的 MySQL/Redis 连接地址需在控制台中配置为实际地址
2. 前端 `.env.production` 中的 `VITE_API_BASE_URL` 已指向云托管后端地址
3. 后端 CORS 已配置允许 CloudBase 静态托管域名跨域
